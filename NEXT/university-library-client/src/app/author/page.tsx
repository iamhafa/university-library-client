...
export default function AuthorPage() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await AuthorApiService.getAll();
      setData(data);
    })();
  }, []);
            ...
            {data.map((item, index) => (
              ...
              >
                ...
                <TableCell className="px-6 py-4">
                  {/* go to detail page */}
                  <Link
                    href={`${EAppRouter.AUTHOR_PAGE}/${item.id}`}
                  >{`${item.firstName} ${item.lastName}`}</Link>
                </TableCell>
                <TableCell className="px-6 py-4 text-gray-600">{item.created_at}</TableCell>
                <TableCell className="px-6 py-4 text-gray-600">{item.updated_at}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
